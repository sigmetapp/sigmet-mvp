-- Update insert_dms_message function to support reply_to_message_id
-- This allows messages to reference other messages for reply functionality

-- Drop old function if exists (with old signature - 6 parameters)
drop function if exists public.insert_dms_message(bigint, uuid, text, text, jsonb, text);

-- Create new function with reply_to_message_id support (7 parameters)
...
begin
  ...
  if p_reply_to_message_id is not null then
    if not exists (
      ...
      raise exception 'Invalid reply_to_message_id: message not found in thread or deleted';
    end if;
  end if;
...
grant execute on function public.insert_dms_message(bigint, uuid, text, text, jsonb, text, bigint) to authenticated;
grant execute on function public.insert_dms_message(bigint, uuid, text, text, jsonb, text, bigint) to service_role;

-- Update comment
comment on function public.insert_dms_message is 
  'Inserts a DM message bypassing RLS policies. Verifies user authentication and thread membership. Supports client_msg_id for deduplication and reply_to_message_id for replies.';
