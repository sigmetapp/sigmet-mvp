-- Create table for storing permanent admin SW adjustments
-- These adjustments persist through SW recalculations
create table if not exists public.admin_sw_adjustments (
  id bigserial primary key,
  user_id uuid not null references auth.users(id) on delete cascade,
  points numeric not null, -- Can be positive (bonus) or negative (penalty)
  reason text,
...
create index if not exists admin_sw_adjustments_user_id_idx 
  on public.admin_sw_adjustments(user_id);

-- Create index for created_at for sorting
create index if not exists admin_sw_adjustments_created_at_idx 
  on public.admin_sw_adjustments(created_at desc);

-- Enable RLS
alter table public.admin_sw_adjustments enable row level security;
...
create policy "admins can read all adjustments" 
  on public.admin_sw_adjustments for select 
...
create policy "admins can insert adjustments" 
  on public.admin_sw_adjustments for insert 
  with check (public.is_admin());

-- Policy: Only admins can update adjustments (for corrections)
create policy "admins can update adjustments" 
  on public.admin_sw_adjustments for update 
  using (public.is_admin())
  with check (public.is_admin());

-- Policy: Only admins can delete adjustments
create policy "admins can delete adjustments" 
  on public.admin_sw_adjustments for delete 
  using (public.is_admin());

-- Function to get total admin adjustments for a user
create or replace function public.get_admin_sw_adjustments_total(target_user_id uuid)
returns numeric
language plpgsql
security definer
stable
...
begin
  ...
  from public.admin_sw_adjustments
