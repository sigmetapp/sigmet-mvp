...
begin
  ...
  if exists (
    ...
    execute 'select exists (select 1 from public.posts where author_id is not null limit 1)'
      into has_blocks_table;
    if has_blocks_table then
      posts_author_col := 'author_id';
    end if;
  end if;

  if posts_author_col is null and exists (
    select 1
    from information_schema.columns
    where table_schema = 'public'
      and table_name = 'posts'
  ...
  -- Resolve comments author column dynamically
  if exists (
    select 1
    from information_schema.columns
    where table_schema = 'public'
    ...
    execute 'select exists (select 1 from public.comments where author_id is not null limit 1)'
      into has_blocks_table;
    if has_blocks_table then
  ...
  if comments_author_col is null and exists (
    select 1
    from information_schema.columns
    where table_schema = 'public'
      and table_name = 'comments'
