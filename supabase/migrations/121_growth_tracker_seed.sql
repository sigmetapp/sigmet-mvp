...
-- Personal growth
union all select (select id from d where slug='personal'), 'habit'::public.task_type,'weekly'::public.habit_period,'Reflect in journal','Write weekly reflection',6,1
...
-- Digital skills
union all select (select id from d where slug='digital'), 'habit'::public.task_type,'weekly'::public.habit_period,'Learn a tool','Study a new digital tool',6,1
...
union all select (select id from d where slug='education'), 'habit'::public.task_type,'monthly'::public.habit_period,'Help someone learn','Mentor or help learning',8,1
...
-- Purpose & Meaning
union all select (select id from d where slug='purpose'), 'habit'::public.task_type,'weekly'::public.habit_period,'Review purpose','Reflect on why weekly',6,1
