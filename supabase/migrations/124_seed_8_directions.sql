-- Seed data: 8 directions with merged tasks from previous 12
-- This replaces the old 12-direction seed
begin;
...
('mindfulness','Mindfulness & Inner Balance','?????',8)
...
-- Mindfulness & Inner Balance (merged from mindfulness and personal)
union all select (select id from d where slug='mindfulness'), 'habit'::public.task_type,'daily'::public.habit_period,'Meditate 5-10m','Short daily meditation',3,1
...
union all select (select id from d where slug='mindfulness'), 'habit'::public.task_type,'weekly'::public.habit_period,'Reflect in journal','Write weekly reflection',6,4
union all select (select id from d where slug='mindfulness'), 'habit'::public.task_type,'weekly'::public.habit_period,'Review goals','Weekly goal review',6,5
union all select (select id from d where slug='mindfulness'), 'habit'::public.task_type,'monthly'::public.habit_period,'Seek feedback','Ask for feedback monthly',8,6
union all select (select id from d where slug='mindfulness'), 'goal'::public.task_type,null,'Retreat or detox','Complete a retreat or digital detox',70,7
union all select (select id from d where slug='mindfulness'), 'goal'::public.task_type,null,'Build a habit','Install a new positive habit',50,8
union all select (select id from d where slug='mindfulness'), 'goal'::public.task_type,null,'Quit a bad habit','Drop a harmful habit',90,9
union all select (select id from d where slug='mindfulness'), 'goal'::public.task_type,null,'Do a personality test','Complete a test or coaching',50,10
union all select (select id from d where slug='mindfulness'), 'goal'::public.task_type,null,'Hit a personal goal','Achieve a personal milestone',80,11
union all select (select id from d where slug='mindfulness'), 'goal'::public.task_type,null,'Courageous step','Do a bold meaningful step',90,12
