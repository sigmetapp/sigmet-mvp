-- Add date_of_birth and work_career_status to profiles
begin;

-- Add columns to profiles table
alter table if exists public.profiles
  add column if not exists date_of_birth date,
  add column if not exists work_career_status text check (work_career_status in ('employed', 'entrepreneur', 'student', 'looking_for_opportunities', 'unemployed'));
...
begin
  ...
  if current_user_id is not null and current_user_id != new.user_id then
    ...
    if coalesce(old.date_of_birth::text, '') != coalesce(new.date_of_birth::text, '') then
      insert into public.profile_changes (target_user_id, editor_id, field_name, old_value, new_value)
      values (new.user_id, current_user_id, 'date_of_birth', old.date_of_birth::text, new.date_of_birth::text);
    end if;
    if coalesce(old.work_career_status, '') != coalesce(new.work_career_status, '') then
      insert into public.profile_changes (target_user_id, editor_id, field_name, old_value, new_value)
      values (new.user_id, current_user_id, 'work_career_status', old.work_career_status, new.work_career_status);
    end if;
