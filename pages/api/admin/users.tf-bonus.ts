...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    // Record permanent adjustment in admin_tf_adjustments table
    ...
    const { data: adjustmentsData, error: adjustmentsError } = await admin
      .rpc('get_admin_tf_adjustments_total', { target_user_id: user_id });

    const adminAdjustmentsTotal = adjustmentsData || 0;
    
    // Calculate new total: current TF + all admin adjustments
    const currentTF = currentProfile?.trust_flow || 5.0;
    const newTF = currentTF + points;
    ...
    console.error('users.tf-bonus error', e);
