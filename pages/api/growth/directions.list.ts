...
export default async function handler(
  ...
  try {
    ...
    if (dirError) {
      ...
      return res
        .status(500)
        .json({ error: dirError.message || "Failed to load directions" });
    }
    ...
    if (selError) {
      ...
      if (selError.code !== "PGRST116" && selError.code !== "42501") {
        return res
          .status(500)
          .json({
            error: selError.message || "Failed to load user selections",
          });
      }
    }

    const selectedIds = new Set(
      (userSelections || []).map((s) => s.direction_id),
    );
    ...
    if (tasksError) {
      ...
      if (tasksError.code !== "PGRST116" && tasksError.code !== "42501") {
        return res
          .status(500)
          .json({ error: tasksError.message || "Failed to load tasks" });
      }
    }
    ...
    if (ledgerError) {
      ...
      if (ledgerError.code !== "PGRST116" && ledgerError.code !== "42501") {
        return res
          .status(500)
          .json({ error: ledgerError.message || "Failed to load ledger" });
      }
    }
