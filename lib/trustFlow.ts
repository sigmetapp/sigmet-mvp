 ...
 */
export async function calculateUserActivityScore(
  userId: string,
): Promise<number> {
  let supabase;
  try {
    ...
    console.error(
      "[Trust Flow] Error creating supabaseAdmin in calculateUserActivityScore:",
      error,
    );
    return 0;
  }
  ...
  try {
    ...
    if (
      postsResult.error &&
      postsResult.error.message?.includes("column") &&
      postsResult.error.message?.includes("author_id")
    ) {
    ...
    if (
      ...
      commentsResult.error.message?.includes("column") &&
      commentsResult.error.message?.includes("author_id")
    ) {
      // Fallback to user_id if author_id doesn't exist
      const fallbackResult = await supabase
        .from("comments")
    ...
    const activityScore =
      (postsCount || 0) + (commentsCount || 0) + Math.floor(swPoints / 100);
...
export async function calculateAccountAgeDays(userId: string): Promise<number> {
  ...
  try {
    ...
    console.error(
      "[Trust Flow] Error creating supabaseAdmin in calculateAccountAgeDays:",
      error,
    );
    return 0;
  }
...
export async function calculateUserWeight(
  ...
  const calculatedWeight =
    Math.log(1 + adjustedActivityScore) * Math.log(1 + adjustedAccountAgeDays);
...
export async function getRepeatCount(
  ...
  try {
    const { data, error } = await supabase.rpc("get_trust_push_repeat_count", {
...
export async function canUserPush(
  ...
  try {
    const { data, error } = await supabase.rpc(
      "get_trust_pushes_count_last_month",
      {
        p_from_user_id: fromUserId,
        p_to_user_id: toUserId,
      },
...
export async function calculateTrustFlowForUser(
  ...
  try {
    ...
    if (pushes) {
      console.log(
        `[Trust Flow] Query returned ${pushes.length} pushes:`,
        JSON.stringify(pushes.slice(0, 5), null, 2),
      );
    }

    const queryEndTime = Date.now();
    console.log(
      `[Trust Flow] Query executed in ${queryEndTime - queryStartTime}ms for user ${userId}`,
    );

    // Double-check: verify the query actually executed and returned data
    if (error) {
      console.error(
        "[Trust Flow] Query error details:",
    ...
    if (error) {
      ...
      console.log(
        `[Trust Flow] Returning BASE_TRUST_FLOW (${BASE_TRUST_FLOW}) due to error`,
      );
      return BASE_TRUST_FLOW;
    }

    let effectivePushes: TrustPush[] | null = pushes as TrustPush[] | null;

    console.log(
      `[Trust Flow] Found ${effectivePushes?.length || 0} pushes for user ${userId}`,
    );
    if (!effectivePushes || effectivePushes.length === 0) {
      console.log(
        `[Trust Flow] No entries in trust_pushes for user ${userId}, checking legacy trust_feedback...`,
      );
      try {
        const { data: legacyFeedback, error: legacyError } = await supabase
          .from("trust_feedback")
        ...
        if (legacyError) {
          console.error(
            "[Trust Flow] Error loading legacy trust_feedback:",
            legacyError,
          );
        } else if (legacyFeedback && legacyFeedback.length > 0) {
          console.log(
            `[Trust Flow] Found ${legacyFeedback.length} legacy trust_feedback entries for user ${userId}`,
          );
          effectivePushes = legacyFeedback.map((entry: any) => ({
            id: -1, // legacy entries do not have push IDs; use placeholder
            from_user_id: entry.author_id,
            to_user_id: userId,
            type: Number(entry.value) >= 0 ? "positive" : "negative",
            reason: entry.comment ?? null,
            context_type: null,
            context_id: null,
        ...
        console.error(
          "[Trust Flow] Exception while converting legacy trust_feedback:",
          legacyException,
        );
      }
    }
    ...
    if (effectivePushes && effectivePushes.length > 0) {
      const latestPush = effectivePushes[effectivePushes.length - 1];
      const latestPushTime = new Date(latestPush.created_at).getTime();
      const now = Date.now();
      const timeSinceLatestPush = now - latestPushTime;
      console.log(
        `[Trust Flow] Latest push: from=${latestPush.from_user_id}, type=${latestPush.type}, created_at=${latestPush.created_at}, time_since=${timeSinceLatestPush}ms`,
      );
      // Log all pushes for debugging
      console.log(
        `[Trust Flow] All pushes:`,
        effectivePushes.map((p) => ({
          from: p.from_user_id,
          type: p.type,
    ...
    if (!effectivePushes || effectivePushes.length === 0) {
      ...
      console.log(
        `[Trust Flow] No pushes found for user ${userId}, returning base value: ${BASE_TRUST_FLOW}`,
      );
      return BASE_TRUST_FLOW;
    }

    console.log(
      `[Trust Flow] Processing ${effectivePushes.length} pushes for user ${userId}`,
    );

    // Group pushes by from_user_id to calculate repeat counts efficiently
    const pushesByUser = new Map<string, TrustPush[]>();
    for (const push of effectivePushes) {
      ...
      weightPromises.push(
        calculateUserWeight(fromUserId)
          .then((data) => {
            ...
            console.log(
              `[Trust Flow] User ${fromUserId} weight: ${data.weight.toFixed(4)}, activity: ${data.activityScore}, age: ${data.accountAgeDays} days`,
            );
          })
          .catch((error) => {
            console.error(
              `[Trust Flow] Error calculating weight for user ${fromUserId}:`,
              error,
            );
    ...
    console.log(
      `[Trust Flow] Weight cache after calculation:`,
      Array.from(weightCache.entries()).map(([uid, w]) => ({
        userId: uid,
        weight: w.toFixed(4),
      })),
    );
    ...
    console.log(
      `[Trust Flow] Starting TF calculation with positiveSum=${positiveSum}, negativeSum=${negativeSum}`,
    );
    ...
    for (const check of firstPushChecks) {
      ...
      console.log(
        `[Trust Flow] First push check for ${check.fromUserId}: isFirstPush=${check.isFirstPush}`,
      );
    }

    console.log(
      `[Trust Flow] Processing ${pushesByUser.size} unique pushers for user ${userId}`,
    );
    ...
    for (const [fromUserId, userPushes] of pushesByUser.entries()) {
      ...
      console.log(
        `[Trust Flow] Processing pushes from user ${fromUserId}, weight: ${weight.toFixed(4)}, push count: ${userPushes.length}`,
      );
      ...
      for (let i = 0; i < sortedPushes.length; i++) {
        ...
        console.log(
          `[Trust Flow] Push ${i + 1}/${sortedPushes.length} from ${fromUserId}: type=${push.type}, repeatCount=${repeatCount}, effectiveWeight=${effectiveWeight.toFixed(4)}, isFirstPushEver=${isFirstPushEver}`,
        );
    ...
    console.log(
      `[Trust Flow] After processing all pushes: positiveSum=${positiveSum.toFixed(4)}, negativeSum=${negativeSum.toFixed(4)}`,
    );
    ...
    console.log(
      `[Trust Flow] Calculation for user ${userId}: positiveSum=${positiveSum.toFixed(2)}, negativeSum=${negativeSum.toFixed(2)}, contributions=${contributions.toFixed(2)}, base=${BASE_TRUST_FLOW}, finalTF=${roundedTF.toFixed(2)}`,
    );
    console.log(
      `[Trust Flow] Weight cache:`,
      Array.from(weightCache.entries()).map(([uid, w]) => ({
        userId: uid,
        weight: w.toFixed(4),
      })),
    ...
    console.log(
      `[Trust Flow] Pushes by user:`,
      Array.from(pushesByUser.entries()).map(([uid, ps]) => ({
        userId: uid,
        count: ps.length,
        types: ps.map((p) => p.type),
      })),
    );
    ...
    console.log(
      `[Trust Flow] Final TF for user ${userId}: ${finalTF.toFixed(2)} (base: ${BASE_TRUST_FLOW}, contributions: ${contributions.toFixed(2)})`,
    );
    ...
    // Return base value on error to ensure users always have a minimum TF
    console.log(
      `[Trust Flow] Returning BASE_TRUST_FLOW (${BASE_TRUST_FLOW}) due to exception`,
    );
    return BASE_TRUST_FLOW;
  }
}
  ...
  try {
    ...
    if (rpcError) {
      ...
      if (
        ...
        console.warn(
          "[Trust Flow] RPC function not found, trying direct update (migration may not be applied)",
        );
        ...
        if (updateError) {
          ...
          if (
            ...
            updateError.message?.includes("trust_flow")
          ) {
            console.warn(
              "[Trust Flow] trust_flow column not found - migration needs to be applied",
            );
          } else {
            console.error(
              "[Trust Flow] Error updating cache directly:",
              updateError,
            );
          }
        } else {
          console.log(
            `[Trust Flow] Saved TF ${trustFlow.toFixed(2)} to cache for user ${userId} (direct update)`,
          );
        }
      } else {
  ...
  try {
    ...
    if (error) {
      ...
      if (
        ...
        error.message?.includes("trust_flow")
      ) {
        console.warn(
          "[Trust Flow] trust_flow column not found - migration may not be applied",
        );
        return null;
      }
