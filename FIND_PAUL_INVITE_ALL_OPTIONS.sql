-- Comprehensive search for Paul's invite - all possible scenarios
...
-- Step 1: Check ALL accepted invites around Paul's registration time
-- This will show all invites accepted within 2 hours of Paul's registration
SELECT 
  i.id as invite_id,
  i.invite_code,
  i.status,
...
WHERE i.status = 'accepted'
  AND i.accepted_at BETWEEN '2025-11-17 18:00:00'::timestamptz AND '2025-11-17 21:00:00'::timestamptz
ORDER BY i.accepted_at DESC;

-- Step 2: Check ALL invite events around Paul's registration time
SELECT 
  ie.id as event_id,
  ie.invite_id,
  ie.event,
...
WHERE ie.event = 'accepted'
  AND ie.created_at BETWEEN '2025-11-17 18:00:00'::timestamptz AND '2025-11-17 21:00:00'::timestamptz
...
-- Step 4: Check ALL accepted invites that don't have consumed_by_user_id set
-- This might be the issue - invite was accepted but consumed_by_user_id wasn't set
SELECT 
  i.id,
  i.invite_code,
  i.status,
...
-- Step 5: Check invites accepted on the same day as Paul registered
...
SELECT 
  ...
  CASE 
    WHEN consumer.user_id = 'c2c8c5e9-d4c6-46bc-a685-5326972b812b' THEN '✅ THIS IS PAUL!'
    WHEN DATE(i.accepted_at) = DATE('2025-11-17'::date) AND consumer.user_id IS NULL THEN '⚠️ Accepted on same day as Paul but consumed_by_user_id is NULL!'
    WHEN DATE(i.accepted_at) = DATE('2025-11-17'::date) THEN '⏰ Accepted on same day as Paul'
    ELSE 'Other'
  END as match_status
FROM public.invites i
LEFT JOIN public.profiles inviter ON i.inviter_user_id = inviter.user_id
LEFT JOIN public.profiles consumer ON i.consumed_by_user_id = consumer.user_id
WHERE i.status = 'accepted'
  AND DATE(i.accepted_at) = DATE('2025-11-17'::date)
ORDER BY i.accepted_at DESC;

-- Step 6: Check if there are any invites where the code might have been used but not properly recorded
-- Look for invites that were created before Paul registered but accepted after
SELECT 
  i.id as invite_id,
  i.invite_code,
  i.status,
