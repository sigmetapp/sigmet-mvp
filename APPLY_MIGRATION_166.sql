-- ============================================
-- Применение миграции 166_optimize_dm_queries.sql
-- ============================================
-- 
-- ШАГ 1: Откатить старую версию (если была применена)
-- ============================================

BEGIN;

-- Удалить старую версию функции (если существует)
DROP FUNCTION IF EXISTS public.dms_list_partners_optimized(UUID, INTEGER, INTEGER);

COMMIT;

-- ============================================
-- ШАГ 2: Применить новую миграцию
-- ============================================

BEGIN;

-- 1. Добавить индексы для оптимизации запросов
...
-- 2. Создать оптимизированную функцию dms_list_partners_optimized
-- Эта функция полностью соответствует оригиналу по сигнатуре
...
-- 1. SELECT * FROM dms_list_partners_optimized('your-user-id'::uuid, 20, 0);
-- 2. Убедитесь, что индексы созданы:
--    SELECT indexname FROM pg_indexes WHERE tablename LIKE 'dms_%';
-- ============================================
