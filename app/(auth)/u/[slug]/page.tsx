...
export default function PublicProfilePage() {
  ...
  const [userGoals, setUserGoals] = useState<
    Array<{ id: string; text: string; target_date: string | null }>
  >([]);
  ...
  const [pushType, setPushType] = useState<"positive" | "negative" | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    if (!showStatus) {
      ...
      console.log(
        '[Online Status] Privacy setting enabled - showing "Private online"',
      );
      setIsOnline(null);
      return;
    }
    ...
    channel
      .on("presence", { event: "sync" }, () => {
        ...
        const finalStatus = checkOnlineStatus(
          presenceOnline,
          profile.last_activity_at,
        );
        console.log(
          "[Online Status] Sync result - presenceOnline:",
          presenceOnline,
          "finalStatus:",
          finalStatus,
        );
      ...
      .on("presence", { event: "join" }, ({ key, newPresences }) => {
        console.log("[Online Status] Presence join event:", {
          key,
          newPresences,
        });
      ...
      .on("presence", { event: "leave" }, () => {
        ...
        console.log(
          "[Online Status] Leave result - presenceOnline:",
          presenceOnline,
          "finalStatus:",
          finalStatus,
        );
    ...
    (async () => {
      try {
        ...
        console.log(
          "[Online Status] Initial check result - presenceOnline:",
          presenceOnline,
          "finalStatus:",
          finalStatus,
        );
      ...
      try {
        ...
        if (data) {
          ...
          if (newLastActivityAt !== profile.last_activity_at) {
            ...
            setProfile((prev) =>
              prev ? { ...prev, last_activity_at: newLastActivityAt } : null,
            );
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { data } = await supabase
        .from("profiles")
        .select(
          "user_id, username, full_name, bio, country, website_url, facebook_url, instagram_url, twitter_url, avatar_url, directions_selected, show_online_status, created_at, last_activity_at, relationship_status, educational_institution_id, date_of_birth, work_career_status",
        )
        .eq("username", slug)
        .maybeSingle();
      ...
      if (profileData?.user_id) {
        try {
          const statusResp = await fetch(
            `/api/user/${profileData.user_id}/status`,
          );
          if (statusResp.ok) {
    ...
    (async () => {
      try {
        ...
        const res = await fetch(
          `/api/users/${profile.user_id}/trust-flow?recalculate=true`,
          {
            headers: {
    ...
    (async () => {
      try {
        const { data } = await supabase
          ...
          .or(
            `follower_id.eq.${profile.user_id},followee_id.eq.${profile.user_id}`,
          )
        ...
        for (const r of rows) {
          if (r.followee_id === profile.user_id) {
            mapped.push({
              kind: "in",
              otherUserId: r.follower_id as string,
              created_at: r.created_at,
            });
          } else if (r.follower_id === profile.user_id) {
            mapped.push({
              kind: "out",
              otherUserId: r.followee_id as string,
              created_at: r.created_at,
            });
          }
        }
      ...
      try {
        ...
        if (followersData.data && followingData.data) {
          const followersSet = new Set(
            followersData.data.map((f: any) => f.follower_id),
          );
          const followingSet = new Set(
            followingData.data.map((f: any) => f.followee_id),
          );
        ...
        if (profileData?.goals && Array.isArray(profileData.goals)) {
          ...
          if (goals.length > 0) {
            ...
            goalIds.forEach((goalId) => {
              const goalReactions = (reactions || []).filter(
                (r) => r.goal_id === goalId && r.kind === "inspire",
              );
              reactionsMap[goalId] = {
                count: goalReactions.length,
                selected: user
                  ? goalReactions.some((r) => r.user_id === user.id)
                  : false,
              };
            });
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const filtered = (badgeTypes || []).filter((bt) =>
          earnedBadgeIds.has(bt.id),
        );
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const isStale =
          !swScore ||
          !swScore.last_updated ||
          Date.now() - new Date(swScore.last_updated).getTime() >
            15 * 60 * 1000;
        ...
        fetch("/api/sw/weights", {
          ...
          .then((weightsData) => {
            if (weightsData?.sw_levels) {
              try {
                const mappedLevels = weightsData.sw_levels.map(
                  (level: any, index: number) => {
                    const defaultLevel =
                      SW_LEVELS.find((l) => l.name === level.name) ||
                      SW_LEVELS[index] ||
                      SW_LEVELS[0];
                    return {
    ...
    try {
      if (!iFollow) {
        const { error } = await supabase
          .from("follows")
          .insert({ follower_id: viewerId, followee_id: profile.user_id });
  ...
  async function onPickAvatar(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      await supabase
        .from("profiles")
        .upsert({ user_id: me, avatar_url: url }, { onConflict: "user_id" });
  ...
  async function submitFeedback(kind: "up" | "down") {
    ...
    if (!feedbackText || feedbackText.trim().length < 100) {
      alert(
        "Please provide a message with at least 100 characters explaining your push.",
      );
      return;
    }
    ...
    try {
      ...
      try {
        const canPushRes = await fetch(
          `/api/users/${profile.user_id}/trust-push/check`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
      ...
      let insertedPushId: number | null = null;
      try {
        ...
        console.log(
          "[Trust Push] Trust push inserted successfully:",
          insertData.id,
          "type:",
          insertData.type,
          "from:",
          insertData.from_user_id,
          "to:",
          insertData.to_user_id,
        );
        insertedPushId = insertData.id;
        ...
        if (verifyError || !verifyData) {
          console.error(
            "[Trust Push] Failed to verify push was saved:",
            verifyError,
          );
          // Continue anyway - the insert might have succeeded even if verify failed
        } else {
          console.log(
            "[Trust Push] Push verified in database:",
            verifyData.id,
            "created_at:",
        ...
        console.error(
          "[Trust Push] Exception inserting trust push:",
          insertErr,
        );
        alert("Failed to submit feedback. Please try again.");
        setFeedbackPending(false);
      ...
      while (retries > 0 && !recalculated) {
        try {
          console.log(
            `[Trust Push] Recalculating Trust Flow (attempt ${4 - retries}/3)...`,
          );
          const recalcUrl = `/api/users/${profile.user_id}/trust-flow?recalculate=true${insertedPushId !== null ? `&pushId=${insertedPushId}` : ""}`;
          ...
          if (res.ok) {
            ...
            const tfValue =
              ...
              data.trustFlow > 0
                ...
                : 5.0; // BASE_TRUST_FLOW fallback
            console.log(
              "[Trust Push] Trust Flow recalculated and cached successfully:",
              tfValue,
              "previous:",
              trustFlow,
            );
            ...
            const errorText = await res.text();
            console.error(
              "[Trust Push] Error recalculating Trust Flow:",
              res.status,
              errorText,
            );
            retries--;
            if (retries > 0) {
              ...
              console.warn(
                "[Trust Push] All retries failed, using base Trust Flow value",
              );
              setTrustFlow(5.0); // BASE_TRUST_FLOW
              setTrustFlowColor("gray");
            }
      ...
      if (!recalculated) {
        console.warn(
          "Failed to recalculate Trust Flow after retries, using base value",
        );
    ...
    try {
      ...
      const [pushesRes, changesRes] = await Promise.all([
        ...
        supabase
          ...
          .select(
            "editor_id, field_name, old_value, new_value, comment, created_at",
          )
      ...
      if (isAdmin) {
        ...
        if (allPushes && allPushes.length > 0) {
          ...
          for (const fromUserId of pushesByUser.keys()) {
            weightPromises.push(
              calculateUserWeight(fromUserId)
                .then((data) => {
                  ...
                  console.log(
                    `[TF History] User ${fromUserId} weight: ${data.weight}, activity: ${data.activityScore}, age: ${data.accountAgeDays} days`,
                  );
                })
                .catch((error) => {
                  console.error(
                    `[TF History] Error calculating weight for user ${fromUserId}:`,
                    error,
                  );
          ...
          const firstPushCheckPromises: Promise<{
            fromUserId: string;
            isFirstPush: boolean;
          }>[] = [];
          for (const [fromUserId, userPushes] of pushesByUser.entries()) {
            ...
            const sortedPushes = [...userPushes].sort(
              (a, b) =>
            ...
            firstPushCheckPromises.push(
              (async () => {
                ...
                const { count } = await supabase
                  ...
                  .lt(
                    "created_at",
                    firstPushTimestamp || new Date().toISOString(),
                  );
          ...
          for (const [fromUserId, userPushes] of pushesByUser.entries()) {
            ...
            for (let i = 0; i < sortedPushes.length; i++) {
              ...
              const isFirstPushEver =
                firstPushMap.get(fromUserId) === true && repeatCount === 0;
              const effectiveWeight = isFirstPushEver
                ? 1.5
                : weight / (1 + repeatCount);
              const contribution =
                push.type === "positive" ? effectiveWeight : -effectiveWeight;
  ...
  return (
    <div className="max-w-7xl mx-auto px-0 md:px-4 py-6 md:py-8 space-y-6">
      {/* Profile header */}
      <div
        className={`card p-4 md:p-6 mx-4 md:mx-0 ${!loadingProfile && profile ? "animate-fade-in-up" : ""}`}
      >
        {loadingProfile ? (
          ...
          <div className="flex flex-col md:flex-row md:items-start items-center gap-4">
              ...
              <div className="relative">
                {(() => {
                  ...
                  const progressToNext = nextLevel
                    ? ((totalSW - currentLevel.minSW) /
                        (nextLevel.minSW - currentLevel.minSW)) *
                      100
                    : 100;
                  ...
                  const colorScheme =
                    LEVEL_COLOR_SCHEMES[currentLevel.name] ||
                    LEVEL_COLOR_SCHEMES["Beginner"];
                      ...
                      >
                        ...
                        <circle
                          ...
                          style={{
                            ...
                            animationDelay: "0.3s",
                            transition:
                              "stroke-dashoffset 1.5s cubic-bezier(0.4, 0, 0.2, 1), opacity 0.6s ease-out 0.3s",
                          }}
                        />
                      </svg>
                      <img
            ...
            <div className="min-w-0 flex-1 w-full animate-fade-in-up animate-stagger-2">
              <div className="flex flex-col md:flex-row md:items-center gap-2 md:gap-3 md:justify-between">
                ...
                {isMe ? (
                  <div className="flex justify-center md:justify-end gap-2 flex-shrink-0">
                    ...
                    <Link
                      href="/profile"
                      className="px-2.5 md:px-3 py-1.5 rounded-lg text-xs md:text-sm border border-white/20 text-white/80 hover:bg-white/10 whitespace-nowrap"
                    >
                      Edit
                    </Link>
                  </div>
              ...
              <div className="text-white/70 text-sm mt-1 flex flex-wrap items-center gap-2">
                <Link
                  href={`/u/${encodeURIComponent(profile.username || profile.user_id)}`}
              ...
              <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                {/* Social Weight */}
                <div
                  className={`rounded-2xl border border-white/15 bg-white/5 p-3 animate-fade-in-up ${!loadingSW && totalSW !== null ? "animate-stagger-3" : ""}`}
                >
                  <div className="flex items-center justify-between text-white/80 text-sm mb-2">
                    ...
                    <div className="flex items-center gap-2">
                      {(() => {
                        ...
                        const textColorMap: Record<string, string> = {
                          "text-gray-400": isLight
                            ? "text-gray-700"
                            : "text-gray-300",
                          "text-blue-400": isLight
                            ? "text-blue-700"
                            : "text-blue-300",
                          "text-purple-400": isLight
                            ? "text-purple-700"
                            : "text-purple-300",
                          "text-yellow-400": isLight
                            ? "text-yellow-700"
                            : "text-yellow-300",
                          "text-orange-400": isLight
                            ? "text-orange-700"
                            : "text-orange-300",
                          "text-pink-400": isLight
                            ? "text-pink-700"
                            : "text-pink-300",
                        };
                        const textColor =
                          textColorMap[currentLevel.color] ||
                          (isLight ? "text-gray-700" : "text-white/80");
                        const badgeClass = `${colorScheme.badgeBorder} ${colorScheme.badgeBg} ${textColor}`;
                        return (
                          <div
                            className={`px-2 py-0.5 rounded-full border text-xs font-medium ${badgeClass}`}
                          >
                            {currentLevel.name}
                          </div>
                        );
                      ...
                      {loadingSW ? (
                        <div className="px-2 py-0.5 rounded-full border border-white/20 text-white/80 text-xs">
                          Loading...
                        </div>
                      ) : totalSW !== null ? (
                ...
                {/* Trust Flow */}
                <div
                  className={`rounded-2xl border border-white/15 bg-white/5 p-3 animate-fade-in-up animate-stagger-4`}
                >
                  <div className="flex items-center justify-between text-white/80 text-sm mb-2">
                    ...
                    <div
                      className={`px-2 py-0.5 rounded-full border border-white/20 text-white/80 ${
                        trustFlowColor === "red"
                          ? "text-red-300"
                          : trustFlowColor === "yellow"
                            ? "text-yellow-300"
                            : trustFlowColor === "green"
                              ? "text-green-300"
                              : trustFlowColor === "blue"
                                ? "text-blue-300"
                                : "text-white/80"
                  ...
                  <div className="h-2 rounded-full bg-white/10 overflow-hidden">
                    <div
                      className="h-full"
                      style={trustBarStyleFor(trustFlow)}
                    />
                  </div>
                  {!isMe && (
                    <button
              ...
              {(() => {
                const primaryDirection = profile.directions_selected?.length
                  ? availableDirections.find(
                      (a) =>
                        profile.directions_selected?.includes(a.id) &&
                        a.isPrimary === true,
                    )
                  : null;
          ...
          <div className="card p-5 md:p-6 mx-4 md:mx-0 animate-fade-in-up animate-stagger-2">
              ...
              <div>
                <div className="space-y-3">
                      ...
                      <div className="flex flex-wrap items-start gap-3">
                        ...
                        {profile.bio && (
                          <>
                            <div className="flex-1 min-w-0">
                              ...
                              <div
                                className={`text-sm leading-snug ${isLight ? "text-primary-text" : "text-white/90"}`}
                              >
                                {profile.bio}
                              </div>
                            </div>
                            {educationalInstitution && (
                              <div
                                className={`w-px h-12 ${isLight ? "bg-gray-300" : "bg-white/20"}`}
                              />
                            )}
                          </>
                        )}
                        ...
                        {educationalInstitution && (
                          <div className="flex-shrink-0">
                            ...
                            <div
                              className={`text-sm ${isLight ? "text-primary-text" : "text-white/80"}`}
                            >
                              {educationalInstitution.name} (
                              {educationalInstitution.type === "school"
                                ? "School"
                                : educationalInstitution.type === "college"
                                  ? "College"
                                  : "University"}
                              )
                            </div>
                    ...
                    <div className="space-y-3">
                      ...
                      {profile.country && (
                        <div>
                          ...
                          >
                            {(() => {
                              const city = String(profile.country)
                                .split(",")[0]
                                .trim();
                              return (
                                <Link
                      ...
                      {profile.work_career_status && (
                        <div>
                          ...
                          <div
                            className={`text-sm font-medium ${isLight ? "text-primary-text" : "text-white/90"}`}
                          >
                            {profile.work_career_status === "employed"
                              ? "Employed"
                              : profile.work_career_status === "entrepreneur"
                                ? "Entrepreneur"
                                : profile.work_career_status === "student"
                                  ? "Student"
                                  : profile.work_career_status ===
                        ...
                        <div>
                          ...
                          >
                            {profile.relationship_status === "single"
                              ? "Single"
                              : profile.relationship_status === "looking"
                                ? "Looking"
                  ...
                  <div className="pt-3 border-t border-white/10">
                    <div
                      className={`text-[10px] font-semibold mb-3 uppercase tracking-wider ${
                        isLight
                          ? "text-primary-text-secondary"
                          : "text-white/50"
                      }`}
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                      ...
                      <div className="flex flex-col">
                        ...
                        <div
                          className={`text-[10px] ${isLight ? "text-primary-text-secondary" : "text-white/60"}`}
                        >
                          Mutual
                        </div>
                      </div>
            ...
            {activeTab === "goals" && (
              <div className="space-y-2">
                {userGoals.length === 0 ? (
                  <div
                    className={`text-center py-12 ${isLight ? "text-primary-text-secondary" : "text-white/60"}`}
                  >
                    <svg
                    ...
                    <p className="text-sm">
                      No goals set yet. Goals can be added in profile settings.
                    </p>
                  </div>
                ) : (
                  <div className="space-y-2">
                    {userGoals.map((goal, index) => (
                      ...
                      >
                        <div className="flex items-center gap-3 flex-1 min-w-0">
                          ...
                          <div
                            className={`text-sm flex-1 min-w-0 ${isLight ? "text-primary-text" : "text-white/90"}`}
                          >
                            {goal.text}
                          </div>
                        </div>
                        <div className="flex items-center gap-3 flex-shrink-0">
                          ...
                          {profile?.user_id && (
                            <GoalReactions
                              ...
                              initialSelected={
                                goalReactions[goal.id]?.selected || false
                              }
      ...
      {feedbackOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          <div
            ...
            onClick={() =>
              !feedbackPending &&
              (setFeedbackOpen(false), setPushType(null), setFeedbackText(""))
            }
          />
              ...
              <div className="flex items-start gap-2 px-3 py-2 rounded-lg border bg-amber-900/20 border-amber-700/30 text-amber-300">
                ...
                <div className="text-sm space-y-1">
                  ...
                  <ul className="list-disc list-inside space-y-0.5 text-xs">
                    <li>
                      <strong>Authenticity:</strong> Provide honest and accurate
                      evaluations based on real interactions and experiences.
                    </li>
                    <li>
                      <strong>Responsibility:</strong> You are responsible for
                      your actions. False or malicious pushes may result in
                      account restrictions.
                    </li>
                    <li>
                      <strong>Anti-Manipulation:</strong> Pushes are limited to
                      prevent gaming the system. Abuse will be detected and
                      penalized.
                    </li>
                    <li>
                      <strong>Appeals:</strong> If you receive a negative push,
                      you can submit an appeal for review by the moderation
                      team.
                    </li>
                  </ul>
                </div>
                ...
                {(() => {
                  ...
                  const shareToInstagram = async () => {
                    ...
                    try {
                      ...
                      alert(
                        "Profile link copied to clipboard! You can paste it in Instagram.",
                      );
                    } catch (err) {
                  ...
                  const shareToTelegram = () => {
                    window.open(
                      `https://t.me/share/url?url=${urlEncoded}&text=${encodeURIComponent(`Check out my profile on Sigmet.app!\n${swText}\n${tfText}`)}`,
                      "_blank",
                      "width=600,height=400",
                    );
                  };
      ...
      {/* Posts */}
      <div
        className={`space-y-4 ${!loadingProfile && profile ? "animate-fade-in-up" : ""}`}
      >
        {!loadingProfile && profile && (
          <h2 className="text-lg text-white/90 animate-fade-in px-4 md:px-0">
            Posts
          </h2>
        )}
        {!loadingProfile && profile && (
...
function RecentSocialItem({
  event,
}: {
  event: { kind: "in" | "out"; otherUserId: string; created_at?: string };
}) {
...
function HistoryRow({
  ...
  const u =
    user?.username || (item.author_id ? item.author_id.slice(0, 8) : "Anon");
  const avatar =
    ...
    return (
      <li className="flex items-start gap-3 px-3 py-2 text-sm">
        ...
        <img
          src={avatar}
          alt="avatar"
          className="h-6 w-6 rounded-full object-cover border border-white/10 flex-shrink-0"
        />
        <div className="flex-1 min-w-0">
          <span className="text-white/80">
            <Link
          ...
          {item.comment && (
            <div className="text-white/60 text-xs mt-1 whitespace-pre-wrap break-words">
              {item.comment}
            </div>
          )}
              ...
              <div className="grid grid-cols-2 gap-x-4 gap-y-1">
                <div>
                  ...
                  <span className="ml-2 text-white/80">
                    {item.tfDetails.weight.toFixed(2)}
                  </span>
                </div>
                <div>
                  <span className="text-white/60">Repeat Count:</span>
                  <span className="ml-2 text-white/80">
                    {item.tfDetails.repeatCount}
                  </span>
                </div>
                <div>
                  <span className="text-white/60">Effective Weight:</span>
                  <span className="ml-2 text-white/80">
                    {item.tfDetails.effectiveWeight.toFixed(4)}
                  </span>
                </div>
                <div>
                  ...
                  <span
                    className={`ml-2 font-medium ${
                      item.tfDetails.contribution >= 0
                        ? "text-emerald-300"
                        : "text-rose-300"
                    }`}
                  >
                    {item.tfDetails.contribution >= 0 ? "+" : ""}
                    {item.tfDetails.contribution.toFixed(4)}
                  </span>
                </div>
              </div>
            </div>
